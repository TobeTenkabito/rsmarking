...
async def handle_modify(payload: AIRequestPayload, db: AsyncSession, model_name: str) -> Dict[str, Any]:
    ...
    if payload.overwrite:
        ...
        await db.commit()
        return {"status": "success", "mode": "modify", "action": "overwrite", "target_id": str(payload.target_id),
        ...
        if payload.data_type == DataType.RASTER:
            ...
            if not original: raise ValueError(f"新建失败：找不到 id={payload.target_id} 的矢量图层")
            ...
            await db.commit()
            return {"status": "success", "mode": "modify", "action": "create_new", "new_layer_id": str(new_layer.id),
                    "modified_data": modified_dict}