...
export const SidebarComponent = {
    ...
    renderItem(raster, isActive, isLoading) {
        ...
        return `
            <div class="layer-item p-3 flex items-center hover:bg-slate-50 transition-all group border-l-4 ${activeItemClass}"
               data-id="${raster.id}">
                ...
                <div class="mr-3 flex items-center justify-center">
                    <div class="relative w-4 h-4">
                        <input type="checkbox"
                             ${checkedAttr}
                             data-id="${raster.id}"
                             class="layer-checkbox w-4 h-4 rounded border-slate-300
                                    text-indigo-600 focus:ring-indigo-500 transition-all cursor-pointer">
                        ${isLoading ? `
                ...
                class="p-1.5 text-slate-300 hover:text-violet-500 hover:bg-violet-50 
                rounded-lg transition-colors"
                title="光谱查询：点击后在地图上拾取像素">
                ...
                <!-- 📊 栅格属性表入口 -->
                <button onclick="event.stopPropagation(); RS.openAttriRaster('${raster.index_id}','${_esc(raster.file_name)}')"
                ...
                class="btn-delete p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50
                rounded-lg transition-colors"
                title="从工作站移除">
        ...
        return `
            <div class="p-3 flex items-center hover:bg-emerald-50/50 transition-all group ${rowCls}"
                ...
                <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    ...
                    <!-- 📊 属性表入口 -->
                    <button onclick="event.stopPropagation(); RS.openAttriVector('${layer.id}', '${layer.name}')"
                            class="p-1.5 text-slate-300 hover:text-indigo-500 hover:bg-indigo-50
                                   rounded-lg transition-colors"
                            title="打开属性表">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
